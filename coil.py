...
class Coil(object):
    ...
    *(coil_depth/wire_thickness)
    ...
    def Flux_Dencity(self,flux_location):
        ...
        small_length = 2 * self.winding_radius * np.tan(np.pi / self._winding_sides)
        ...
        for i in range(self._winding_sides):
            new_radius_vector = np.dot(rotation_matrix(self.coil_axis, i * (2 * np.pi / self._winding_sides)), start_radius_vector)
